...
import { AIRequestEventType, } from './ai-session-events.js';
...
const createEmitter = mitt.default ?? mitt;
/**
...
export class AISessionBase {
    ...
    constructor(params) {
         ...
         */
        this._state = AISessionState.unknown;
        /**
        ...
        this.heartbeatReady = new Promise((r) => {
    ...
    async *sendGenerator(request) {
        ...
        const chatHeartbeatIntervalId = setInterval(() => {
            try {
                ...
                this.logger.log(LogLevel.warn, `[AISession][...${this.id.slice(-4)}] sendHeartbeat failed: ${error.message || 'Unknown Error'}`);
            }
        }, WS_TIMEOUT / 2.1);
        const queue = [];
        const pendingChunks = new Map();
        const latestInferenceChunk = { output_index: -1 };
        let nextChunkIndex = 0;
        let responseBegan = false;
        let errored = false;
        let closed = false;
        let result = '';
        let pullResolver = null;
        const timeoutTtfb = Math.min(request.requestConfig?.timeoutTtfb || MAX_TTFB_TIMEOUT, MAX_TTFB_TIMEOUT);
        const timeoutResult = Math.min(request.requestConfig?.timeoutResult || MAX_RESULT_TIMEOUT, MAX_RESULT_TIMEOUT);
        ...
        socket.onmessage = (ev) => {
            const raw = ev.data;
            this.logger.log(LogLevel.verbose, `[AISession][...${this.id.slice(-4)}] Received chunk: "${raw}"`);
            if (ACK_MESSAGE === raw) {
            ...
            if (json.content?.startsWith(DONE_SENTINEL)) {
                queue.push({
        ...
        try {
            ...
            socket.send(JSON.stringify(this.prepareQuery({
                chat_seq: ++this.requestCount,
                query: payloadStr,
            })));
            this.logger.log(LogLevel.log, `[AISession][...${this.id.slice(-4)}] Sent inference request: ${payloadStr}`);
            while (true) {
                ...
                if (queue.length > 0) {
                    ...
                    if (nextChunk) {
                        ...
                        if (AIRequestEventType.error === nextChunk.type &&
                            [
                                nextChunk.message?.toLowerCase(),
                                nextChunk.error?.message?.toLowerCase(),
                            ].some((message) => message.includes(SESSION_IS_CLOSED.toLowerCase()))) {
                            this.logger.log(LogLevel.error, `[AISession][...${this.id.slice(-4)}] Encountered session is closed error, closing...`);
                            await this.close();
                        }
                    }
                    continue;
                ...
                if (closed || errored)
                    break;
                await new Promise((r) => (pullResolver = r));
            }
        }
        finally {
    ...
    async close() {
        this.logger.log(LogLevel.log, `[AISession][...${this.id.slice(-4)}] close()`);
        if ([AISessionState.closing, AISessionState.closed].includes(this.state)) {
            this.logger.log(LogLevel.verbose, `[AISession][...${this.id.slice(-4)}] Session (${this.state}) already closing or closed: ${this.state}`);
        }
        else {
    ...
    async startHeartbeat() {
        ...
        this.heartbeatIntervalId = setInterval(() => {
            this.sendHeartbeatOnce().catch((error) => {
                this.logger.log(LogLevel.error, `[AISession][...${this.id.slice(-4)}] Caught error sending heartbeat: `, error);
            });
        }, 5000);
    }
        ...
        if (!this.hbWs) {
            ...
            setTimeout(() => {
                if (AISessionState.opening === this.state) {
                    this.logger.log(LogLevel.verbose, `[AISession][...${this.id.slice(-4)}] Optimistically setting state to open.`);
                    this.setState(AISessionState.open);
                }
            }, this.optimisticHbConnectionTimeout);
            socket.onmessage = () => {
                if (AISessionState.opening === this.state) {
                    this.logger.log(LogLevel.verbose, `[AISession][...${this.id.slice(-4)}] Initial heartbeat response received, setting state to open.`);
                    this.setState(AISessionState.open);
                }
            };
            this.hbWs = socket;
    ...
    sendHeartbeat(socket, key) {
        ...
        this.logger.log(LogLevel.verbose, `[AISession][...${this.id.slice(-4)}] Sent heartbeat (${pathname}) ${hb.message}:${hb.signature_message.length}`);
    }
    stopHeartbeat() {
        ...
        this.closeSocket(this.hbWs);
        this.hbWs = null;
        this.logger.log(LogLevel.log, `[AISession][...${this.id.slice(-4)}] Stopped heartbeat`);
    }
    async openSocket({ url, protocols, timeout, }) {
        this.logger.log(LogLevel.verbose, `[AISession][...${this.id.slice(-4)}] Opening socket (${url})`);
        const pathname = new URL(url).pathname;
        return new Promise((resolve, reject) => {
            const socket = new WebSocket(url, protocols);
            const timer = timeout &&
                setTimeout(() => reject(new Error(`[AISession][...${this.id.slice(-4)}] WebSocket (${pathname}) timed out`)), timeout);
            socket.onopen = () => {
                timeout && clearTimeout(timer);
                this.logger.log(LogLevel.log, `[AISession][...${this.id.slice(-4)}] WebSocket (${pathname}) opened`);
                resolve(socket);
            };
            socket.onerror = (ev) => {
            ...
            socket.onclose = (ev) => {
                if (socket.readyState !== WebSocket.OPEN) {
                    reject(new Error(`[AISession][...${this.id.slice(-4)}] Websocket (${pathname}) closed prematurely (code=${ev.code}, reason=${ev.reason})`));
                }
            };
        });
    }
    closeSocket(socket) {
        try {
            if (socket && socket.readyState !== WebSocket.CLOSED) {
                socket.close();
                this.logger.log(LogLevel.log, `[AISession][...${this.id.slice(-4)}] WebSocket ${new URL(socket.url).pathname} closed`);
            }
        }
        catch {
            /* empty */
...
//# sourceMappingURL=ai-session-base.js.map