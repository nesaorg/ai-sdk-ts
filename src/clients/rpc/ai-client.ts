...
} from './signing-rpc-client.js';
import { Logger, LogLevel, noopLogger } from '../../utils/logger.js';
...
> {
   ...
   */
  private constructor(rpc: SigningRpcClient, logger: Logger) {
    super({ logger });
    this.rpc = rpc;
  }
  ...
  public static async initialize(
    config: AIClientRpcConfiguration,
  ): Promise<AIClientRpc> {
    const {
      rpcEndpoint,
      signer,
    ...
    logger.log(
      LogLevel.log,
      `[AIClient] Connecting on-chain with RPC ${rpcEndpoint}`,
    );
    const rpcClient = await SigningRpcClient.connectWithSigner(
      rpcEndpoint,
      signer,
      senderAddress,
      chainId,
      prefix || 'nesa',
      defaultDenom || 'unes',
      defaultGasPrice || '0.025unes',
      { ...config.clientOptions, logger },
    );
  ...
  public async createSession(
    params: AIClientRpcSessionInitParams,
  ): Promise<AISessionRpc> {
    const { modelName, sessionSeed, optimisticHbConnectionTimeout } = params;
    this.logger.log(
      LogLevel.log,
      `[AIClient] createSession("${modelName}", lock=${params.lockAmount})`,
    );

    const keyData = await generateKey();
    const onChain: RegisterSessionOnChain = await this.rpc.registerSession(
      keyData,
      params,
    );
    ...
    this.logger.log(
      LogLevel.log,
      `[AIClient] Session created on-chain: sessionId=${onChain.sessionId}, txHash=${onChain.transactionHash}`,
    );
    ...
    if (!agentRecord || !agentRecord.url) {
      throw new Error(
        `[AIClient] Unable to locate InferenceAgent for account=${onChain.account}, model="${modelName}"`,
      );
    }

    const chatId =
      sessionSeed ?? toHex(crypto.getRandomValues(new Uint8Array(32)));

    const { chat, heartbeat } = this.constructInferenceUrls(
      agentRecord.url,
      chatId,
    );
