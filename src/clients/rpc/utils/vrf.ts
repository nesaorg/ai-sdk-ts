...
export async function generateKey(): Promise<VRFKeyData> {
...
export function processVRF(
  vrfSeed: Uint8Array<ArrayBufferLike>,
  keyData: VRFKeyData,
) {
  ...
  return {
    vrf: { seed: vrfSeed, proof, hashRandom: hash } as VRF,
...
export function generateEphemeralKeyPair(): {
  ...
  const privateKeyBytes = new Uint8Array(
    keyPair.getPrivate().toArray('be', 32),
  );
  const publicKeyCompressed = new Uint8Array(
    keyPair.getPublic().encodeCompressed(),
  );
  return { privateKeyBytes, publicKeyCompressed };
}

export function Evaluate(privateKey: any, m: any): [Uint8Array, Uint8Array] {
  // eslint-disable-next-line import/no-named-as-default-member
  const ec = new elliptic.ec('secp256k1');
  ...
  return [index, buf as unknown as Uint8Array];
}
