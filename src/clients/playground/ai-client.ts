...
export class AIClientPlayground extends AIClientBase<
  AIClientPlaygroundSessionInitParams,
  AISessionPlayground
> {
  /**
   * Type identifier for this client implementation (always 'playground').
   */
  public readonly type = 'playground' as const;
   ...
   */
  private constructor(agentUrl: string, authToken: string, logger: Logger) {
    super({ logger });
  ...
  public static async initialize(
    config: AIClientPlaygroundConfiguration,
  ): Promise<AIClientPlayground> {
  ...
  public async createSession(
    params: AIClientPlaygroundSessionInitParams,
  ): Promise<AISessionPlayground> {
    const {
      modelName: inModelName,
      sessionSeed,
      optimisticHbConnectionTimeout,
    } = params;

    const modelName = inModelName.toLowerCase();
    this.logger.log(
      LogLevel.log,
      `[AIClient] createSession("${modelName}", seed=${sessionSeed})`,
    );
